"""
LLM Evaluation Framework with AWS Bedrock Guardrails
Supports AWS Bedrock and OpenAI models with comprehensive guardrails evaluation
"""
...
from simplified_evaluator import SimplifiedEvaluator
from simplified_report_generator import SimplifiedReportGenerator
from guardrails_integration import GuardrailsIntegration

load_dotenv()


class LLMEvaluationFramework:
    """Main evaluation framework with guardrails support"""
    ...
    def __init__(self):
        ...
        self.guardrails = GuardrailsIntegration()
        self.report_generator = SimplifiedReportGenerator()
        
        print(f"\n{'='*70}")
        print(f"LLM EVALUATION FRAMEWORK WITH GUARDRAILS")
        print(f"{'='*70}")
        print(f"Test Models: {self.config['test_models']}")
        print(f"Guardrails: {'Enabled' if self.guardrails.enabled else 'Disabled'}")
        if self.guardrails.enabled:
            print(f"Guardrails Mode: {self.guardrails.mode.upper()}")
            print(f"Configured Guardrails: {len(self.guardrails.guardrails)}")
            for gr in self.guardrails.guardrails:
                print(f"  - {gr['id']} (v{gr['version']})")
    ...
    def _evaluate_single_model(self, model_config: Dict, categories: List[str]) -> List:
        ...
        for category in categories:
            ...
            for idx, template in enumerate(templates, 1):
                ...
                print(f"  Test {idx}/{len(templates)}: {template['name']}", end=' ')
    ...
    def _print_model_summary(self, results: List[Dict], model_config: Dict):
        ...
        print(f"  Blocked: {blocked} ({(blocked/total*100) if total > 0 else 0:.1f}%)")
    ...
    def generate_reports(self) -> Dict[str, str]:
        ...
        # Framework info for reports
        framework_info = {
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'test_models': [f"{m['provider']}/{m['model_id']}" for m in self.config['test_models']],
            'guardrails_enabled': self.guardrails.enabled,
            'guardrails_configured': self.guardrails.guardrails if self.guardrails.enabled else []
        }
        
        # PDF Report
        pdf_path = os.path.join(
            self.config['output_path'],
            f"guardrails_evaluation_report_{timestamp}.pdf"
        )
        ...
        json_path = os.path.join(
            self.config['output_path'],
            f"guardrails_evaluation_data_{timestamp}.json"
        )
...
def main():
    ...
    print("Starting LLM Evaluation Framework with Guardrails...")
